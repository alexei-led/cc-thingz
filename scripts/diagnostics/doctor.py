...
def inspect(
    ...
    for plugin in plugins:
        ...
        if expected:
            scan.add(
                "package-version",
                "passed" if plugin["version"] == expected["version"] else "failed",
                f"Installed {plugin['version']}; source {expected['version']}",
                [path],
            )
            for skill in expected["skills"]:
                ...
                required = [skill_path / "SKILL.md"]
                source = repo / "src/skills" / skill
                required.extend(
                    skill_path / helper.relative_to(source)
                    for helper in sorted((source / "scripts").rglob("*"))
