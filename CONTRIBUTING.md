...
- **One source of truth: `src/`.** Hand-edit only files under `src/skills/`,
  ...
  `linguist-generated=true` so GitHub hides it in diffs. `AGENTS.md`
  is hand-maintained — do not rely on the compiler to regenerate it.
...
├── AGENTS.md                          # Hand-maintained — agent/skill catalog for all targets
...
The compiler (`scripts/build/compile.py`) is the single entry point for all
derivative output. `make build` calls it; so does the pre-push hook.

### Targets and layouts

Four targets are compiled on every run. Layout controls how output is grouped
under `dist/<target>/`:

- `claude` — plugin layout: each skill/agent/hook lands under
  `dist/claude/plugins/<plugin>/<type>/<name>/`. Hooks are grouped by plugin
  and their manifests (`hooks.json`) are generated per plugin.
- `codex` — plugin layout: same grouping, with agents also emitted as `.toml`
  files via `codex_toml.py`.
- `gemini` — flat layout: skills at `dist/gemini/skills/<name>/`, agents at
  `dist/gemini/agents/<name>`, hooks at `dist/gemini/hooks/`. A single
  `hooks.json` covers all hooks. Root-level symlinks (`skills/`, `agents/`,
  `hooks/`) point into `dist/gemini/` so Gemini finds them.
- `pi` — flat layout: same flat structure, plus `dist/pi/extensions/` for
  compiled TypeScript Pi extensions.

Plugin membership is declared in `src/plugins/<plugin>/plugin.yaml`. For
plugin-layout targets, a skill not listed in any `plugin.yaml` is skipped
(it can't land at an addressable path). For flat-layout targets, all skills
compile regardless of plugin membership.

### Per-skill compilation steps

For each `src/skills/<name>/` × each target:

1. Load `SKILL.md` — split YAML frontmatter and body text.
2. Check `targets:` frontmatter key. When present and the current target is
   not listed, skip this target entirely. Skills with no `targets:` key
   compile to all four targets.
3. Merge `<target>/frontmatter.yaml` (if present) onto the base frontmatter
   using deep merge. The `targets:` key is stripped before writing.
4. Apply `<target>/body.md` (if present). Auto-detects mode:
   - Full replacement — no shared headers with the base; base body is dropped.
   - Mirror mode — shared headers act as anchors; overlay sections are
     spliced in; unmatched overlay headers are appended.
5. Inject the per-target preamble (`scripts/build/preambles/`):
   - `claude`: no preamble.
   - `codex`, `gemini`: `platform.md` prepended to body.
   - `pi`: `pi.md` prepended to body.
6. Copy base support files (`references/`, `scripts/`, `assets/`) verbatim to
   each output directory, then layer per-target overrides from `<target>/`.
7. Write the assembled `SKILL.md` to every resolved output directory.

Skills with no platform subdirs at all compile identically across all four
targets — the base `SKILL.md` is used as-is, with only the target preamble
differentiating the outputs. `sequential-thinking` is the canonical example:
no `claude/`, `codex/`, `gemini/`, or `pi/` subdir; `dist/` contains four
identically-bodied `SKILL.md` files (different preamble only).

### Agents and hooks

Agents follow the same overlay model (`AGENT.md` + optional per-target
`body.md`/`frontmatter.yaml`). Codex additionally receives a `.toml` file
generated from frontmatter by `codex_toml.py`.

Hooks are copied to each target's hooks directory. Per-target hook manifests
(`hooks.json` for Claude and Gemini, `codex.hooks.json` for Codex) are
generated from `meta.yaml`.

### What `make build` regenerates

- All of `dist/`
- `.claude-plugin/marketplace.json`
- `.agents/plugins/marketplace.json`
- `gemini-extension.json`
- Root-level symlinks (`skills/`, `agents/`, `hooks/`)

`AGENTS.md` is **not** regenerated by the compiler — it is hand-maintained.
Update it manually when adding, removing, or recategorising agents and skills.
