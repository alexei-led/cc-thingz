...
init_hook_input() {
	...
	if command_exists python3 && python3 -c 'import json,sys
try:
    data=json.loads(sys.stdin.read() or "{}")
except Exception:
    sys.exit(1)
sys.exit(0 if data.get("event") == "stop" and isinstance(data.get("piEvent"), dict) else 1)
' <<<"$HOOK_INPUT_JSON" 2>/dev/null; then
		PI_RUNTIME=1
	fi
}
