...
// Agent Bundler does not render native hook manifests. Keep the hook-runner
// unit fixture optional so the retained runtime tests do not require a custom
// compiler or a generated native file.
...
const generatedHooksFile = Bun.file(generatedHooksConfigPath);
const bundledHooksConfig = (await generatedHooksFile.exists())
	? await generatedHooksFile.text()
	: JSON.stringify({
			hooks: {
				SessionStart: [{ hooks: [{ type: "command", command: "${PI_HOOKS_DIR}/session-start.py", timeout: 5 }] }],
				UserPromptSubmit: [{ hooks: [{ type: "command", command: "${PI_HOOKS_DIR}/skill-enforcer.sh", timeout: 15 }] }],
				PreToolUse: [
					{ matcher: "Write|Edit|MultiEdit", hooks: [{ type: "command", command: "${PI_HOOKS_DIR}/file-protector.py", timeout: 10 }] },
					{ matcher: "Bash", hooks: [{ type: "command", command: "${PI_HOOKS_DIR}/git-guardrails.sh", timeout: 10 }] },
					{ matcher: "ExitPlanMode", hooks: [{ type: "command", command: "${PI_HOOKS_DIR}/revdiff-plan-review.py", timeout: 1740 }] },
				],
				PostToolUse: [{ matcher: "Write|Edit|MultiEdit", hooks: [{ type: "command", command: "${PI_HOOKS_DIR}/smart-lint.sh", timeout: 120 }] }],
				Stop: [{ hooks: [{ type: "command", command: "${PI_HOOKS_DIR}/test-runner.sh", timeout: 120 }] }],
				Notification: [{ hooks: [{ type: "command", command: "${PI_HOOKS_DIR}/notify.sh", timeout: 10, async: true }] }],
			},
		});
