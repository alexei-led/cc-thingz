...
describe("isSafeCommand", () => {
	it.each([
		...
		["npm list --depth=0", false],
		["jq . file.json", true],
		["find . -name '*.ts' -type f", true],
		["git -C /tmp/repo status --short", true],
		["head -n 20 'some file.txt'", true],
		["rg --files -g '*.ts'", true],
		["cat -- -filename", true],
		["find . -delete", false],
		["find -- . -delete", false],
		["sort *", false],
		["__proto__", false],
		["git constructor", false],
		["find . -exec echo test +", false],
		["curl -o output https://example.com", false],
		["git branch new-branch", false],
		["git remote add origin somewhere", false],
		["git diff --output=result", false],
		["git -c core.pager=sh log", false],
		["git log --ext-diff", false],
		["git show --textconv", false],
		["rg --pre=sh pattern .", false],
		["rg --hostname-bin=sh pattern .", false],
		["sort -o result file", false],
		["uniq source destination", false],
		["uniq -- source destination", false],
		["uniq -c source destination", false],
		["sort source destination", true],
		["diff source destination", true],
		["awk 'BEGIN { system(\"touch file\") }'", false],
		["env sh -c true", false],
		["ls; touch file", false],
		["ls && touch file", false],
		["cat README.md | sh", false],
		["echo $(touch file)", false],
		["ls `touch file`", false],
		["ls\ntouch file", false],
		["cat <(touch file)", false],
